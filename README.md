...
* [`src/cljs/hzn_pubs_spa/`](src/cljs/hzn_pubs_spa/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/hzn_pubs_spa/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `hzn-pubs-spa` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/hzn_pubs_spa/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns hzn-pubs-spa.example
  (:require [hzn-pubs-spa.config :as config])
